...
- **15.2 Enterprise Auth, OIDC / OAuth2 & Chromatix Visual Passkeys (Opt-in RBAC) · ✅**: Turnkey SSO integration (GitHub, Google, Keycloak, Generic OIDC, Mock dev users), air-gapped **Chromatix Pixel Standard (CPS)** visual authentication with HMAC-signed PNG badges and sealed timestamps, cookie session management, zero-cost disabled-by-default architecture, and `ctx.user()` / `ctx.has_role()`.
  - **Files**: `crates/grio/src/auth.rs`, `crates/grio/src/context.rs`, `crates/grio/src/app.rs`, `crates/grio/src/server.rs`, `crates/grio/src/lib.rs`, `crates/grio/examples/enterprise_sso.rs`, `crates/grio/examples/chromatix_passkey.rs`, `CHROMATIX_AUTH.md`, `crates/grio/tests/api_predict.rs`.
